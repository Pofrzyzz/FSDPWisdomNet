...
function NavBar() {
  ...
  // Check if a user is logged in (assuming a successful login stores 'userId' in localStorage)
  const isLoggedIn = Boolean(localStorage.getItem("userId"));

  const handleLogout = () => {
    // Clear all user details from localStorage (or remove only specific keys)
    localStorage.clear();
    // Optionally, you could also navigate to the login page or homepage after logout:
    navigate("/LoginPage"); // Change the path as needed
  };
        ...
        <div className="container mx-auto flex flex-col justify-center py-2 px-6 pl-32 h-28">
          ...
          <div className="flex justify-between items-center py-1 border-t h-20">
            <div className="flex space-x-20">
              ...
              {isLoggedIn ? (
                <button onClick={handleLogout} className="text-red-500 font-bold hover:text-red-800">
                  Logout
                </button>
              ) : (
                <a href="LoginPage" className="text-red-500 font-bold hover:text-red-800">
                  Login
                </a>
              )}
            </div>
